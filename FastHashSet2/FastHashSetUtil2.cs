...
namespace FastHashSet2
{
	...
	public partial class FastHashSet2<T>
	{
			...
			if (isHashing)
			{
				for (int i = 0; i < indexArray.Length; i++)
				{
					...
					if (index != NullIndex)
					{
						...
						while (nodeArray[index].nextIndex != NullIndex)
						{
							index = nodeArray[index].nextIndex;
							itemsInChain++;
						}
		...
		public void CreateNodeArrayWithAdjacentChainedNodes()
		{
			if (isHashing)
			{
				TNode[] newNodeArray = new TNode[nodeArray.Length];
