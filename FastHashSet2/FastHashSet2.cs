...
namespace FastHashSet2
{
	public partial class FastHashSet2<T> : ICollection<T>, IEnumerable<T>, IEnumerable, IReadOnlyCollection<T>, ISet<T> //, ISerializable, IDeserializationCallback - ??? implement this within #define
    {
		private const int InitialItemsTableSize = 16;

		private const int LargestPrimeLessThanMaxInt = 2147483629;

		private const int NullIndex = 0; // this indicates end of chain if the nextIndex of a node has this value and also indicates no chain if an indexArray element has this value
		...
		private static readonly int[] indexArraySizeArray = { 7, 17, 47, 89, 173, 347, 691, 1367, 2741, 5471, 10_937, 21_851, 43_711, 87_383, 174_767,
			262_147, 699_053, 1_398_107, 2_796_221, 5_592_407, 11_184_829, 22_369_661, 44_739_259, 89_478_503, 17_8956_983, 35_7913_951, 715_827_947, 143_1655_777, LargestPrimeLessThanMaxInt};
		...
		private int usedItemsLoadFactorThreshold; // maybe remove this is we always resize both arrays at the same time???
		...
		public FastHashSet2(IEnumerable<T> collection)
		{
			comparer = EqualityComparer<T>.Default;
		...
		public FastHashSet2(IEqualityComparer<T> comparer)
		{
		...
		public FastHashSet2(int capacity)
		{
			comparer = EqualityComparer<T>.Default;
		...
		public FastHashSet2(IEnumerable<T> collection, IEqualityComparer<T> comparer)
		{
			this.comparer = comparer ?? EqualityComparer<T>.Default;
			AddInitialEnumerable(collection);
		}
		...
		public FastHashSet2(int capacity, IEqualityComparer<T> comparer)
		{
			this.comparer = comparer ?? EqualityComparer<T>.Default;
			SetInitialCapacity(capacity);
		}
		...
		public FastHashSet2(IEnumerable<T> collection, bool areAllCollectionItemsDefinitelyUnique = false, int initialCapacity = -1, int initialArraySize = -1, IEqualityComparer<T> comparer = null)
		{
		...
		{
			FastHashSet2<T> fhset = collection as FastHashSet2<T>;
			...
			else
			{
				...
				if (nextBlankIndex >= firstBlankAtEndIndex) // we know there aren't any blanks within the nodes, so skip the if (<not blank node>) check
				{
					for (int i = 1; i <= lastNodeIndex; i++)
					{
						...
						int hashIndex = t.hashOrNextIndexForBlanks % indexArray.Length;
		...
		public bool Add(in T item)//??? do we need 2 versions, one with in and one without - if we do then change this one to match the regular Add, which has been tested
		{
			#if !Exclude_Check_For_Set_Modifications_In_Enumerator
			incrementForEverySetModification++;
			#endif
			...
			if (isHashing)
			{
				return AddToHashSetIfNotFound(in item, item == null ? 0 : (comparer.GetHashCode(item) & HighBitNotSet)) != NullIndex;
			}
			else
			{
				int i;
		...
		public bool Contains(T item)
		{
			if (isHashing)
			{
				return FindInNodeArray(in item, item == null ? 0 : (comparer.GetHashCode(item) & HighBitNotSet));
			}
			else
			{
		...
		private FoundType FindInNodeArrayAndMark(in T item, out int foundNodeIndex)
		{
			...
			else
			{
				...
				while (true)
				{
					...
					if (comparer.Equals(t.item, item))
					#else
					if (t.hashOrNextIndexForBlanks == hash && comparer.Equals(t.item, item))
					#endif
					{
...
someItemsInOther:
		...
		public bool SetEquals(IEnumerable<T> other)
		{
			...
			if (c != null)
			{
				...
				FastHashSet2<T> fhset = other as FastHashSet2<T>;
				if (fhset != null && Equals(fhset.Comparer, Comparer))
				{
		...
		private class FastHashSetEqualityComparer : IEqualityComparer<FastHashSet2<T>>
		{
			public bool Equals(FastHashSet2<T> x, FastHashSet2<T> y)
			{
				if (x == null && y == null)
				{
					return true;
			...
			public int GetHashCode(FastHashSet2<T> set)
			{
				if (set == null)
				{
		...
		public static IEqualityComparer<FastHashSet2<T>> CreateSetComparer()
		{
		...
		private class FastHashSetEnumerator<T2> : IEnumerator<T2>
		{
			...
			public FastHashSetEnumerator(FastHashSet2<T2> set)
			{
				this.set = set;
				if (set.isHashing)
				{
